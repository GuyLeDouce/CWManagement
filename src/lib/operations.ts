...
export const assignmentSchema = z.discriminatedUnion('action', [
  z
    .object({
      ...
      projectId: id,
      userId: id,
      role: z.enum(ProjectAssignmentRole),
...
export const projectContactSchema = z.discriminatedUnion('action', [
  z
    .object({
      ...
      projectId: id,
      contactId: id,
      role: z.enum(ProjectContactRole),
      primary: z.boolean().default(false),
    })
    .strict(),
  z
...
export const projectTaskSchema = z
  .object({
    ...
    description: text(),
    status: z.enum(ProjectTaskStatus).default('NOT_STARTED'),
    startDate: date,
    endDate: date,
    ...
    contactIds: z.array(id).max(50).default([]),
    predecessorId: id.optional().nullable(),
    dependencyType: z.enum(DependencyType).default('FINISH_TO_START'),
    lagDays: z.coerce.number().int().min(-365).max(365).default(0),
  })
  .strict()
  .refine((v) => !v.startDate || !v.endDate || v.endDate >= v.startDate, {
    message: 'Finish date must not be before start date.',
    path: ['endDate'],
  });
...
export const dailyLogSchema = z
  ...
  .refine(
    ...
    { message: 'Add at least one daily log entry.' },
  );
export const fileActionSchema = z
  .object({ action: z.literal('archive'), projectId: id, id })
  .strict();
  ...
  return transaction(async (tx) => {
    ...
    if (input.action === 'add') {
      ...
      await publishProjectEvent(tx, {
        ...
        description: `assigned ${user.firstName} ${user.lastName} as ${input.role.toLowerCase().replaceAll('_', ' ')}`,
        after: assignment,
      });
      if (user.id !== actor.id)
        await notify(tx, {
    ...
    const assignment = await tx.projectAssignment.findFirst({
      where: { id: input.assignmentId, projectId: input.projectId },
      include: { user: true },
    });
    ...
    await publishProjectEvent(tx, {
      ...
      entity: 'ProjectAssignment',
      entityId: assignment.id,
      description: `removed ${assignment.user.firstName} ${assignment.user.lastName} from ${assignment.role.toLowerCase().replaceAll('_', ' ')}`,
      before: assignment,
    });
    return { ok: true };
  });
...
export async function saveProjectContact(
  actor: Actor,
  input: z.infer<typeof projectContactSchema>,
) {
  ...
  return transaction(async (tx) => {
    ...
    if (input.action === 'add') {
      const association = await tx.projectContact.upsert({
        where: {
      ...
      await publishProjectEvent(tx, {
        ...
        entityId: contact.id,
        description: `added ${contact.firstName} ${contact.lastName} as ${input.role.toLowerCase().replaceAll('_', ' ')}`,
        after: association,
      });
      return association;
    }
    const association = await tx.projectContact.findUnique({
      where: {
        projectId_contactId_role: {
          projectId: input.projectId,
          contactId: input.contactId,
    ...
    await publishProjectEvent(tx, {
      ...
      entity: 'ProjectContact',
      entityId: contact.id,
      description: `removed ${contact.firstName} ${contact.lastName} as ${input.role.toLowerCase().replaceAll('_', ' ')}`,
      before: association,
    });
    return { ok: true };
  });
...
async function ensureNoDependencyCycle(
  ...
  const dependencies = await tx.projectTaskDependency.findMany({
    where: { predecessor: { projectId } },
  });
  const edges = new Map<string, string[]>();
  for (const item of dependencies)
    edges.set(item.predecessorId, [...(edges.get(item.predecessorId) ?? []), item.successorId]);
  const stack = [successorId],
    visited = new Set<string>();
  while (stack.length) {
    const current = stack.pop()!;
    ensure(current !== predecessorId, 'This dependency would create a schedule cycle.');
...
export async function saveProjectTask(actor: Actor, input: z.infer<typeof projectTaskSchema>) {
  ...
  return transaction(async (tx) => {
    ...
    const task = taskId
      ? await tx.projectTask.update({ where: { id: taskId }, data: fields })
      : await tx.projectTask.create({ data: { ...fields, createdById: actor.id } });
    ...
    if (predecessorId) {
      await ensureNoDependencyCycle(tx, project.id, predecessorId, task.id);
      await tx.projectTaskDependency.create({
        data: { predecessorId, successorId: task.id, type: dependencyType, lagDays },
      });
    }
    const action = before
      ? before.status !== 'COMPLETE' && task.status === 'COMPLETE'
    ...
    await publishProjectEvent(tx, {
      ...
      entity: 'ProjectTask',
      entityId: task.id,
      description: `${verb} schedule task “${task.name}”`,
      before,
      after: task,
    });
    for (const userId of [...new Set(userIds)].filter((value) => value !== actor.id))
      await notify(tx, {
        userId,
        projectId: project.id,
        type: 'TASK_ASSIGNED',
        title: `Task assigned: ${task.name}`,
        message: `You were assigned a schedule task on ${project.name}.`,
...
export async function actOnProjectTask(actor: Actor, input: z.infer<typeof taskActionSchema>) {
  ...
  return transaction(async (tx) => {
    const project = await requireProjectAccess(actor, input.projectId, tx);
    const before = await tx.projectTask.findFirst({
      where: { id: input.id, projectId: input.projectId, archivedAt: null },
    });
    ensure(before, 'Schedule task not found.', 404);
    const task = await tx.projectTask.update({
      where: { id: input.id },
      data:
        input.action === 'archive'
          ? { archivedAt: new Date() }
          : { status: 'COMPLETE', actualEndDate: new Date() },
    });
    await publishProjectEvent(tx, {
      projectId: project.id,
      actorId: actor.id,
      action: input.action === 'archive' ? 'PROJECT_TASK_ARCHIVED' : 'PROJECT_TASK_COMPLETED',
      entity: 'ProjectTask',
      entityId: task.id,
      description: `${input.action === 'archive' ? 'archived' : 'completed'} schedule task “${task.name}”`,
      before,
      after: task,
    });
    return task;
  ...
  return db.dailyLog.findMany({
    ...
    include: {
      ...
      attachments: {
        ...
        select: { id: true, kind: true, originalFilename: true },
      },
    },
...
export async function saveDailyLog(actor: Actor, input: z.infer<typeof dailyLogSchema>) {
  ...
  return transaction(async (tx) => {
    ...
    const log = logId
      ...
      : await tx.dailyLog.create({ data: { ...fields, authorId: actor.id } });
    await publishProjectEvent(tx, {
      projectId: project.id,
      actorId: actor.id,
      action: before ? 'DAILY_LOG_UPDATED' : 'DAILY_LOG_CREATED',
      entity: 'DailyLog',
      entityId: log.id,
      description: `${before ? 'updated' : 'created'} daily log for ${log.date.toISOString().slice(0, 10)}`,
      before,
      after: log,
    });
    return log;
...
export async function files(actor: Actor, projectId: string, kind?: FileKind) {
  await requireProjectAccess(actor, projectId);
  await requireCapability(actor, 'FILE_VIEW_INTERNAL');
  const rows = await db.storedFile.findMany({
    where: { projectId, archivedAt: null, ...(kind ? { kind } : {}) },
...
export async function archiveFile(actor: Actor, input: z.infer<typeof fileActionSchema>) {
  ...
  return transaction(async (tx) => {
    ...
    const before = await tx.storedFile.findFirst({
      where: { id: input.id, projectId: input.projectId, archivedAt: null },
    });
    ...
    await publishProjectEvent(tx, {
      projectId: input.projectId,
      actorId: actor.id,
      action: record.kind === 'PHOTO' ? 'PHOTO_ARCHIVED' : 'FILE_ARCHIVED',
      entity: 'StoredFile',
      entityId: record.id,
      description: `archived ${record.kind === 'PHOTO' ? 'photo' : 'file'} “${record.originalFilename}”`,
      before: { ...before, size: before.size.toString() },
...
export async function updateNotifications(
  actor: Actor,
  input: { id?: string; all?: boolean; unread?: boolean },
) {
  ...
  ensure(result.count === 1, 'Notification not found.', 404);
  return { ok: true };
}
